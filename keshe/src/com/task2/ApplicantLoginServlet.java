...
public class ApplicantLoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int applicantID = dao.login(applicantname,password);
		//判读注册自增型id是否已经成功插入
		if(applicantID > 0){
			//将用户的信息存入会话对象
			Applicant applicant = new Applicant(applicantID, applicantname, password);
			request.getSession().setAttribute("SESSION_APPLICANT",applicant);
			rememberMe(rememberMe, applicantname, password, request, response);
